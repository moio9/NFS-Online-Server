...
PollHandler = Callable[[], Iterable[DatagramReply]]
...
class UDPListener:
    ...
    def _dispatch_poll_replies(self) -> None:
        if self.poll_handler is None:
            return
        try:
            self._dispatch_replies(self.poll_handler())
        except Exception:
            log.exception("%s UDP poll handler failed", self.name)
